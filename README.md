...
The current ten Labs are the initial content set. They deliberately include observable baseline failures: reproduce the symptom, form a hypothesis, use tests and logs, make a small change, and explain the trade-off. See each local README for candidate guidance, acceptance criteria, Hint 1/2/3, guided debugging, mentor spoilers, troubleshooting and validation evidence.
...
| [Lab 09 — Frontend Experience Engineering](./lab-09-frontend-experience-engineering/) | Vue 3, Angular, TypeScript, Playwright, axe                            | Turning a mediocre CMS-driven frontend into a professional one: visual implementation, responsive UI, third-party library evaluation, WCAG 2.2, SEO, Schema.org and frontend QA.                                                           |
| [Lab 10 — LeetCode Interview Simulation](./lab-10-leetcode-interview/)                | Python + pytest, Java 21 + JUnit 5 + Spring Boot, FastAPI/SQLite or H2 | A multi-stage hiring simulation in two parallel language tracks: an algorithmic online assessment, a DP/BFS technical screen, and an onsite-style round combining live coding, debugging, system design and product/behavioral discussion. |
...
These nine Labs are the initial content, not a closed curriculum. Any contributor can add Easy, Intermediate, Advanced or specialised challenges to any Lab. New work should preserve the canonical directory structure, deterministic local execution, independent observable defects, progressive hints, acceptance criteria, tests, troubleshooting, mentor guidance and Agent Continuity described in [LAB_SPEC](docs/LAB_SPEC.md). See [CONTRIBUTING.md](CONTRIBUTING.md) for the contribution workflow and [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) for community expectations.
